...
- Default mode uses server VAD.
- Session config sets `audio.input.turn_detection.type` to `server_vad`.
- Pressing the mic button starts a live WebRTC session; pressing it again closes the session and releases microphone tracks.
...
- The UI exposes a voice mode toggle: `Open mic` and `Push to talk`.
...
- Spacebar and the mic button can both act as push controls when the text input is not focused.
